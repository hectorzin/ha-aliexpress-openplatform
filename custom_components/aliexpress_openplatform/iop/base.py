...
def sign(secret, api, parameters):
    ...
    if "/" in api:
        parameters_str = "%s%s" % (
            api,
            "".join("%s%s" % (key, parameters[key]) for key in sort_dict),
        )
    else:
        parameters_str = "".join("%s%s" % (key, parameters[key]) for key in sort_dict)
...
class IopClient:
    ...
    def execute(self, request, access_token=None):
        ...
        elif self.log_level == P_LOG_LEVEL_DEBUG or self.log_level == P_LOG_LEVEL_INFO:
